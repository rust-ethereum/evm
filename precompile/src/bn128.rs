...
pub fn run_add<G: GasMutState>(
	...
	Ok((ExitSucceed::Returned, output.into()))
}

/// Run the Bn254 mul precompile
pub fn run_mul<G: GasMutState>(
	input: &[u8],
	gas_cost: u64,
	gasometer: &mut G,
) -> Result<(ExitSucceed, Vec<u8>), ExitError> {
...
#[inline]
fn g1_point_mul(point_bytes: &[u8], fr_bytes: &[u8]) -> Result<[u8; 64], ExitError> {
