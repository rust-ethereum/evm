...
pub fn create_machine(v: &Value, block: &JSONBlock) -> SeqVM {
    ...
    SeqVM::new(transaction, block.block_header(), Patch::None)
}
    ...
    if callcreates.as_array().is_some() {
        ...
        for callcreate in callcreates.as_array().unwrap() {
            ...
            if transaction.gas_limit != gas_limit || transaction.value != value || transaction.data != data {
                if debug {
                    print!("\n");
                    println!("Transaction mismatch. gas limit 0x{:x} =?= 0x{:x}, value 0x{:x} =?= 0x{:x}", transaction.gas_limit, gas_limit, transaction.value, value);
                }
                return false;
            }
