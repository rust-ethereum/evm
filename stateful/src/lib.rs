...
macro_rules! stateful {
    ($name: tt, $database: tt) => {
        ...
        impl<G> $name<G> {
            pub fn new(database: $database, code_hashes: G, root: H256) -> Self {
                Self {
                    database,
                    code_hashes,
                    root
        ...
        impl<G: Default> Default for $name<G> {
            fn default() -> Self {
                Self {
...
stateful!(GenericMemoryStateful, MemoryDatabase);
pub type MemoryStateful = GenericMemoryStateful<HashMap<H256, Vec<u8>>>;
