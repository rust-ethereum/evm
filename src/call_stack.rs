use crate::interpreter::{HandleFor, Interpreter, StepInterpreter, TrapFor};
...
// Drop.
struct CallStack<'backend, 'invoker, I: Invoker> {
	stack: Vec<Substack<I::Interpreter, I::SubstackInvoke>>,
	#[allow(clippy::type_complexity)]
	last: Option<LastSubstack<I::Interpreter, TrapFor<I::Interpreter>>>,
	initial_depth: usize,
	backend: &'backend mut HandleFor<I::Interpreter>,
	invoker: &'invoker I,
}

impl<'backend, 'invoker, I> CallStack<'backend, 'invoker, I>
where
	I: Invoker,
{
	pub fn new(
	...
	where
		FS: Fn(
			&mut I::Interpreter,
			&mut HandleFor<I::Interpreter>,
		) -> LastSubstackStatus<TrapFor<I::Interpreter>>,
	{
...
fn execute<I>(
	...
	backend: &mut HandleFor<I::Interpreter>,
	invoker: &I,
) -> Result<(ExitResult, I::Interpreter), ExitFatal>
where
	I: Invoker<Interrupt = Infallible>,
{
...
enum HeapTransactState<'backend, 'invoker, I: Invoker> {
	Created {
		args: I::TransactArgs,
		invoker: &'invoker I,
		backend: &'backend mut HandleFor<I::Interpreter>,
	...
	Running {
		call_stack: CallStack<'backend, 'invoker, I>,
...
pub struct HeapTransact<'backend, 'invoker, I: Invoker>(
	Option<HeapTransactState<'backend, 'invoker, I>>,
);

impl<'backend, 'invoker, I> HeapTransact<'backend, 'invoker, I>
where
	I: Invoker,
{
	...
	where
		FS: Fn(
			&mut CallStack<'backend, 'invoker, I>,
		) -> Result<
			(),
			Capture<Result<(ExitResult, I::Interpreter), ExitFatal>, I::Interrupt>,
		>,
...
impl<'backend, 'invoker, I> Drop for HeapTransact<'backend, 'invoker, I>
where
	I: Invoker,
{
	fn drop(&mut self) {
