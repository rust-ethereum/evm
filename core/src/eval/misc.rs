...
pub fn codesize(state: &mut Core) -> Control {
    let size = U256::from(state.code.len());
    push_u256!(state, size);
    Control::Continue(1)
}

pub fn codecopy(state: &mut Core) -> Control {
...
pub fn pop(state: &mut Core) -> Control {
    pop!(state, _any);
    Control::Continue(1)
}

pub fn mload(state: &mut Core) -> Control {
...
pub fn mstore(state: &mut Core) -> Control {
    pop_u256!(state, index);
    pop!(state, value);
    let index = as_usize_or_fail!(index);
...
pub fn mstore8(state: &mut Core) -> Control {
...
pub fn jump(state: &mut Core) -> Control {
    pop_u256!(state, dest);
    let dest = as_usize_or_fail!(dest, ExitReason::InvalidJump);
    Control::Jump(dest)
}

pub fn jumpi(state: &mut Core) -> Control {
...
pub fn pc(state: &mut Core, position: usize) -> Control {
    push_u256!(state, U256::from(position));
    Control::Continue(1)
}

pub fn msize(state: &mut Core) -> Control {
    push_u256!(state, U256::from(state.memory.len()));
    Control::Continue(1)
}

pub fn push(state: &mut Core, n: usize, position: usize) -> Control {
...
pub fn dup(state: &mut Core, n: usize) -> Control {
...
pub fn swap(state: &mut Core, n: usize) -> Control {
...
pub fn ret(state: &mut Core) -> Control {
...
pub fn revert(state: &mut Core) -> Control {
    pop_u256!(state, start, len);
    if let Some(end) = start.checked_add(len) {
        state.return_range = start..end;
